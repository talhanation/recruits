...
public class MessengerAnswerScreen extends RecruitsScreenBase {
    ...
    public void renderForeground(PoseStack matrixStack, int mouseX, int mouseY, float delta) {
        ...
        font.draw(matrixStack, "From:", guiLeft + 9, guiTop + 9, fontColor, false);
        font.draw(matrixStack, "To:", guiLeft + 9,  guiTop + 20, fontColor, false);
        font.draw(matrixStack, "" + owner, guiLeft + 50,  guiTop + 9, fontColor, false);
        font.draw(matrixStack, "" + targetPlayer, guiLeft + 50,  guiTop + 20, fontColor, false);

        font.draw(matrixStack, "Time: " + time + unit, guiLeft + 130, guiTop + 9, fontColor, false);

        if(!messenger.getMainHandItem().isEmpty()){
            itemrenderer.renderFakeItem(messenger.getMainHandItem(), guiLeft + 120, guiTop + ySize - 48);
            itemRenderer.renderItemDecorations(font, messenger.getMainHandItem(),guiLeft + 120, guiTop + ySize - 48);
        }
    }
}