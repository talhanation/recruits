...
final class WorldMapRenderTile {
    ...
    private WorldMapRenderTile(WorldMapRenderTileKey key, int sourceVersion, WorldMapTextureAtlas.Slot slot,
                               int[] pixels) {
        this.key = key;
    ...
    static WorldMapRenderTile publish(WorldMapRenderTileKey key, WorldMapRegionTile.RenderSnapshot snapshot,
                                      WorldMapTextureAtlas.Slot slot, WorldMapTextureUploader uploader) {
        ...
        try {
            ...
            WorldMapRegionTile.releaseRenderPixels(pixels);
            throw exception;
        }
    }
    ...
    void update(WorldMapRegionTile.RenderSnapshot snapshot, WorldMapTextureUploader uploader) {
    ...
    void patchChunk(int[] chunkPixels, int chunkXInRegion, int chunkZInRegion, int sourceVersion) {
        int scale = 1 << key.level();
        int patchSize = WorldMapRegionTile.PIXELS_PER_CHUNK / scale;
        int targetX = (chunkXInRegion * WorldMapRegionTile.PIXELS_PER_CHUNK / scale)
                % WorldMapRenderTileKey.PIXEL_SIZE;
        int targetZ = (chunkZInRegion * WorldMapRegionTile.PIXELS_PER_CHUNK / scale)
                % WorldMapRenderTileKey.PIXEL_SIZE;
        ...
        for (int patchZ = 0; patchZ < patchSize; patchZ++) {
            ...
            for (int patchX = 0; patchX < patchSize; patchX++) {
                pixels[targetRow + patchX] = averageArea(
                        chunkPixels,
    ...
    private static int averageArea(int[] source, int sourceSize, int startX, int startZ, int areaSize) {
        int alphaSum = 0;
        long redSum = 0L;
        long greenSum = 0L;
        long blueSum = 0L;
