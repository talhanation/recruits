...
final class WorldMapDebugProfiler {
    ...
    static boolean measureRenderDetails() {
    ...
    static void recordRegionPrefetch(long nanos) {
    ...
    static void recordLodTrim(long nanos) {
    ...
    static void recordTextureUpload(long nanos, boolean publish) {
    ...
    static void recordTileManagerState(int loadedRegions, int pendingRegionLoads, int failedRegionLoads,
                                       int pendingRegionSaves, int chunkQueueSize, int queuedChunkCount,
                                       int lodTileCount, int pendingLodTiles) {
    ...
    static void recordAsyncTask(String type, String detail, long queueWaitNanos, long runNanos, boolean success) {
        if (success && queueWaitNanos < ASYNC_QUEUE_SPIKE_NANOS && runNanos < ASYNC_RUN_SPIKE_NANOS) return;
        ...
        Main.LOGGER.warn(
                ...
                type, detail, ms(queueWaitNanos), ms(runNanos), success, memoryLine()
        );
    }

    static void recordChunkWork(long nanos) {
        tileUpdateChunkWorkNanos += nanos;
    }

    static void recordChunkFinalize(long totalNanos, long regionWriteNanos, long lodInvalidateNanos) {
    ...
    static void recordTileUpdate(long nanos, int chunkUpdates, int chunkQueueSize, int queuedChunkCount,
        ...
        if ((nanos >= TILE_UPDATE_SPIKE_NANOS || allocatedBytes >= TILE_UPDATE_ALLOCATION_SPIKE_BYTES)
            ...
            Main.LOGGER.warn(
                    ...
                    CURRENT.loadedRegions, CURRENT.pendingRegionLoads, CURRENT.pendingRegionSaves, CURRENT.failedRegionLoads,
                    CURRENT.lodTileCount, CURRENT.pendingLodTiles, gcDelta.count(), gcDelta.millis(), allocatedMb(allocatedBytes),
                    memoryLine(), tileImageMemoryLine(CURRENT)
            );
        }
    }
    ...
    private static String coverageLine(Metrics metrics) {
        return "missingTiles=" + metrics.missingTiles
                + " fallback=" + metrics.missingFallbacks
                + " root=L" + metrics.rootLevel
                + " scale=" + String.format(Locale.ROOT, "%.3f", metrics.scale);
    }
    ...
    static long allocatedSince(long startBytes) {
    ...
    private static final class Metrics {
        ...
        private List<String> renderProblemReasons() {
            ...
            if (textureUploadNanos >= TEXTURE_UPLOAD_SPIKE_NANOS) reasons.add("textureUpload=" + ms(textureUploadNanos));
            if (maxWorldTileDrawNanos >= SINGLE_TILE_DRAW_SPIKE_NANOS) reasons.add("singleDraw=" + ms(maxWorldTileDrawNanos));
            ...
            if (tileUpdateGcCount > 0) reasons.add("updateGc=" + tileUpdateGcCount + "/" + tileUpdateGcMillis + "ms");
        ...
        private long estimatedTileImageMb() {
            ...
            long regionBytes = (long) WorldMapRegionTile.REGION_PIXEL_SIZE
                    * WorldMapRegionTile.REGION_PIXEL_SIZE
                    * 4L;
            long renderTileImageCount = lodTileCount + pendingLodTiles;
            long renderTileBytes = (long) WorldMapRenderTileKey.PIXEL_SIZE
        ...
        private String toLogLine() {
            return String.format(Locale.ROOT,
                    ...
                    memoryLine(), tileImageMemoryLine(this));
        }
    }
}
