...
public final class WorldMapBuildProfiler {
    ...
    private static final ThreadLocal<ChunkMetrics> ACTIVE =
            ThreadLocal.withInitial(ChunkMetrics::new);
    private static long lastLogNanos;

    private WorldMapBuildProfiler() {}

    public static void beginChunk(int chunkX, int chunkZ) {
    ...
    public static void recordSamplePhase(long nanos) {
        ChunkMetrics metrics = ACTIVE.get();
        if (metrics.active) metrics.sampleNanos += nanos;
    }

    public static void recordRenderPhase(long nanos) {
        ChunkMetrics metrics = ACTIVE.get();
        if (metrics.active) metrics.renderNanos += nanos;
    }

    public static void recordColorPreparePhase(long nanos) {
        ChunkMetrics metrics = ACTIVE.get();
        if (metrics.active) metrics.colorPrepareNanos += nanos;
    }

    public static void recordTextureColorMiss(long nanos, String blockName) {
        ChunkMetrics metrics = ACTIVE.get();
        if (!metrics.active) return;
    ...
    private static final class ChunkMetrics {
        ...
        private String toLogLine() {
            return String.format(
                    Locale.ROOT,
                    "chunk %d,%d | wall %s | sample %s colors %s render %s other %s | textureMiss %d/%s max"
                            + " %s block %s",
                    chunkX,
                    chunkZ,
                    ms(wallNanos),
                    ms(sampleNanos),
                    ms(colorPrepareNanos),
                    ms(renderNanos),
                    ms(Math.max(0L, wallNanos - sampleNanos - colorPrepareNanos - renderNanos)),
                    textureColorMisses,
                    ms(textureColorMissNanos),
                    ms(maxTextureColorMissNanos),
                    slowestTextureBlock);
        }
    }
}
