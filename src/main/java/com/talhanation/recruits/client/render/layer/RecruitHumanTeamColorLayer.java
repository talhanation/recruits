...
public class RecruitHumanTeamColorLayer extends RenderLayer<AbstractRecruitEntity, PlayerModel<AbstractRecruitEntity>> {
    ...
    private static final ResourceLocation LOCATION = new ResourceLocation(Main.MOD_ID,"textures/entity/human/human_team_cloth.png");
    private static final ResourceLocation LOCATION2 = new ResourceLocation(Main.MOD_ID,"textures/entity/human/human_base_cloth.png");
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int p_117722_, AbstractRecruitEntity recruit, float p_117724_, float p_117725_, float p_117726_, float p_117727_, float p_117728_, float p_117729_) {
        if(!recruit.isInvisible()){
            if (recruit.isOwned() && recruit.getTeam() != null) {
                float[] afloat = recruit.getDyeColor().getTextureDiffuseColors();
                renderColoredCutoutModel(this.getParentModel(), LOCATION, poseStack, bufferSource, p_117722_, recruit, afloat[0], afloat[1], afloat[2]);
            }
            else{
                VertexConsumer vertexconsumer = bufferSource.getBuffer(RenderType.entityCutoutNoCull(LOCATION2));
