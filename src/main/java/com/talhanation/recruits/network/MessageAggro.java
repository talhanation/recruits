...
public class MessageAggro implements Message<MessageAggro> {
    ...
    public void executeServerSide(NetworkEvent.Context context){
        if (fromGui){
            ...
            List<AbstractRecruitEntity> list = Objects.requireNonNull(context.getSender()).getCommandSenderWorld().getEntitiesOfClass(AbstractRecruitEntity.class, context.getSender().getBoundingBox().inflate(100.0D));
            for (AbstractRecruitEntity recruits : list) {
                CommandEvents.onAggroCommand(this.player, recruits, this.state, group, fromGui);
            }
        }
